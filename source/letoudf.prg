...
STATIC FUNCTION Dir_GetFiles( cPath, cMask, cAttr, aFiles )
   Local aDir, aFil, nLen, cSep := hb_ps()
   Local cDataPath := leto_GetAppOptions( 1 )

   IF right(cDataPath, 1) != cSep ; cDataPath += cSep
   ENDIF
   
   Default aFiles := {}, cPath := "", cMask := "*", cAttr := "AD"
...
FUNCTION UDF_ZipFile( cFile, aFiles, nLevel, lOverWrite, cPassword, lWithPath, lWithDrive )
   Local lCompress, cFiles
   Local cCompress := ""
   Local cDataPath := hb_DirSepAdd( leto_GetAppOptions( 1 ) )

   IF Empty( cFile ) .or. !hb_IsChar( cFile ) 
      cCompress := "*** Zip file named not set"
      RETURN cCompress
   ENDIF

   cFile := cDataPath + cFile

   IF Empty( nLevel ) ; nLevel := 7
   ENDIF

   lOverWrite := !Empty( lOverWrite )
   lWithPath  := !Empty( lWithPath  )
   lWithDrive := !Empty( lWithDrive )

   IF hb_IsChar ( aFiles )
      cFiles := cDataPath + aFiles
      IF hb_FileExists( cFiles )
         lCompress := hb_ZipFile( cFile, cFiles, nLevel, , lOverWrite, ;
                                  cPassword, lWithPath, lWithDrive )
         IF !lCompress
            cCompress := "*** Zip file named not created"
         ENDIF
      ELSE
         cCompress := "*** File named to compress not found"
      ENDIF

   ELSEIF hb_IsArray( aFiles ) .and. Len( aFiles ) > 0
      lCompress := hb_ZipFile( cFile, aFiles, nLevel, , lOverWrite, ;
                               cPassword, lWithPath, lWithDrive, )
      IF !lCompress
         cCompress := "*** Zip file named not created"
      ENDIF
